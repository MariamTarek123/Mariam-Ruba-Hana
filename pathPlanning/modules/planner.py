...
class PathPlanner:
    def __init__(
        ...
        self.ghost_filter_enabled = ghost_filter_enabled
    ...
    def execute_cycle(self, cone_data, car_data):
        ...
        # 3. Filter Ghost Cones (Isolated false positives)
        if self.ghost_filter_enabled:
            filtered_cone_data = filters.remove_ghost_cones(
                cone_data,
        ...
        if len(filtered_cone_data) < 3:
            return self._handle_low_cones(filtered_cone_data, car_pos, car_yaw)

        # 4. Balance Uneven Cones (Add ghosts if one side is missing)
        ...
        try:
            yellow_count = sum(1 for c in filtered_cone_data if c[2] == 'y')
            blue_count = sum(1 for c in filtered_cone_data if c[2] == 'b')
